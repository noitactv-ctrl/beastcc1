...
**How to apply:** Resolve managed provider settings on the server at use time, retain documented environment variables only as fallback when no saved override exists, and reject legacy plaintext secrets until they can be encrypted. If a stored encrypted value cannot be decrypted, treat the provider as unavailable for public status endpoints and pause background work rather than returning a 500 or repeatedly logging the same failure. Provider URLs that receive credentials should be restricted to trusted HTTPS origins to prevent key forwarding or SSRF.