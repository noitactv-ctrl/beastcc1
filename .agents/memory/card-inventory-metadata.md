...
Every valid card inventory item should track its six-digit BIN and must include a valid two-letter US state and five-digit ZIP at stock-entry time. Posted state and ZIP remain the source of location details; provider enrichment is optional. Card type filters must be generated from returned BIN types, including PREPAID and future values.
