...
export default function CartPage() {
  ...
  const processorFee = Math.round(cartTotal * CRYPTO_FEE_PERCENT / 100);
        ...
        <Card className="sticky top-24 border-primary/20 shadow-lg shadow-primary/5">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg border border-primary/30 bg-primary/5">
              <div className="h-6 w-6 rounded-full bg-amber-500 flex items-center justify-center flex-shrink-0">
                <SiBitcoin className="h-4 w-4 text-white" />
              </div>
              <div>
                <p className="font-medium text-sm">Crypto Payment</p>
                <p className="text-xs text-muted-foreground">Pay securely with crypto</p>
              </div>
            </div>
          </CardContent>
          <CardFooter className="flex-col gap-3">
            <Button
              ...
              disabled={cryptoOrderMutation.isPending}
              onClick={handleCheckout}
              data-testid="button-proceed-payment"
            >
              {cryptoOrderMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Proceed to Payment
            </Button>
