...
import { Check, ShoppingCart, SlidersHorizontal, X } from "lucide-react";
  ...
  const cardDetails = (card: any, index: number, compact = false) => {
    ...
    return compact ? (
      <article key={card.id} className="border-[3px] border-black bg-[#10215e] p-4 shadow-[3px_3px_0_#050505]">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 font-mono text-[10px]"><span className="bg-[#0a1645] px-2 py-2 text-white/65">TYPE <b className="text-white">{String(card.binData?.type || "—").toUpperCase()}</b></span><span className="bg-[#0a1645] px-2 py-2 text-white/65">BASE <b className="text-white">{card.baseName || "—"}</b></span><span className="bg-[#0a1645] px-2 py-2 text-[#72df7c]">VALID {card.hrPercent ?? 80}%</span><span className="game-check"><Check className="h-3 w-3" /> REFUNDABLE</span></div>
      ...
      <tr key={card.id} className="border-b border-black/40 text-white/80 transition-colors hover:bg-[#19377e]">
        <td className="px-3 py-3 font-mono text-white/55">#{index + 1}</td><td className="px-3 py-3 font-bold text-[#ffe177]">{brand}</td><td className="px-3 py-3 font-mono text-[#ffe177]">{cardBin(card) || "—"}</td><td className="px-3 py-3">{String(card.binData?.type || "—").toUpperCase()}</td><td className="px-3 py-3">{flagFor(code)} {code || "—"}</td><td className="px-3 py-3">{card.baseName || "—"}</td><td className="px-3 py-3 text-[#72df7c]">{card.hrPercent ?? 80}%</td><td className="px-3 py-3"><span className="game-check"><Check className="h-3 w-3" /> YES</span></td><td className="px-3 py-3 font-bold text-[#ffe177]">${(Number(card.price || 0) / 100).toFixed(2)}</td><td className="px-3 py-3"><div className="flex gap-2"><button onClick={() => addToCart(card)} className="pixel-button inline-flex items-center gap-1 px-2 py-2 text-[8px]"><ShoppingCart className="h-3 w-3" /> ADD</button><button onClick={() => addToCart(card, true)} className="pixel-button !bg-[#ee292b] px-2 py-2 text-[8px] !text-white">BUY</button></div></td>
      </tr>
    );
  };
    ...
    <div className="pixel-page space-y-5">
      ...
      <section className="pixel-panel bg-[#10215e] p-4 sm:p-5">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          <label className="filter-field-label">BASE<select className={selectClass} value={filters.base} onChange={e => setFilter("base", e.target.value)}><option value="all">ALL BASES</option>{bases.map((item: any) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          <label className="filter-field-label">COUNTRY<select className={selectClass} value={filters.country} onChange={e => setFilter("country", e.target.value)}><option value="all">ALL COUNTRIES</option>{values.countries.map(code => <option key={code} value={code}>{flagFor(code)} {code}</option>)}</select></label>
          <label className="filter-field-label">BRAND<select className={selectClass} value={filters.brand} onChange={e => setFilter("brand", e.target.value)}><option value="all">ALL BRANDS</option>{values.brands.map(item => <option key={item} value={item}>{item}</option>)}</select></label>
          <div className="filter-field-label">PRICE RANGE<div className="mt-2 grid grid-cols-2 gap-2"><input className="pixel-input h-11 min-w-0 text-xs" inputMode="decimal" value={filters.minPrice} onChange={e => setFilter("minPrice", e.target.value)} placeholder="MIN $" /><input className="pixel-input h-11 min-w-0 text-xs" inputMode="decimal" value={filters.maxPrice} onChange={e => setFilter("maxPrice", e.target.value)} placeholder="MAX $" /></div></div>
