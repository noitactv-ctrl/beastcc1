...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const NavContent = () => (
    <div className="flex flex-col h-full bg-[#080914] text-white overflow-y-auto border-r border-indigo-500/10">
      <div className="px-4 py-5 border-b border-white/5">
        <span className="text-lg font-bold tracking-tight">
          <span className="text-white">NYC</span><span className="text-primary">HQ</span>
        </span>
      </div>
      ...
      <div className="flex flex-col gap-6 p-4 flex-1">
        {navSections.map((section) => (
          <div key={section.label}>
            <p className="text-[10px] font-bold text-white/25 tracking-widest mb-2">{section.label}</p>
              ...
              {section.items.map((item) => {
                ...
                if ((item as any).external) {
                  return (
                    <a
                      ...
                      className="flex items-center gap-3 px-3 py-2 rounded-lg text-xs text-white/50 hover:text-white hover:bg-white/5 transition-all"
                    >
                ...
                return (
                  <Link key={item.href} href={item.href} onClick={() => setIsMobileOpen(false)}>
                    <div className={`flex items-center gap-3 px-3 py-2 rounded-lg text-xs transition-all cursor-pointer ${
                      isActive
  ...
  return (
    <div className="min-h-screen bg-[#09091a]">
      {/* Top Bar */}
      <header className="h-11 border-b border-white/5 bg-[#09091a] sticky top-0 z-40 px-3 flex items-center justify-between">
        {/* Left: Hamburger */}
        <Sheet open={isMobileOpen} onOpenChange={setIsMobileOpen}>
          <SheetTrigger asChild>
            <button className="h-7 w-7 flex items-center justify-center text-white/40 hover:text-white hover:bg-white/8 rounded-lg transition-colors" data-testid="btn-menu">
              <Menu className="h-4 w-4" />
            </button>
          </SheetTrigger>
          <SheetContent side="left" className="w-[240px] p-0 border-r border-indigo-500/10 bg-[#080914]">
            <NavContent />
          </SheetContent>
        </Sheet>

        {/* Center: Brand */}
        <span className="absolute left-1/2 -translate-x-1/2 text-sm font-bold tracking-tight">
          <span className="text-white">NYC</span><span className="text-primary">HQ</span>
        </span>
        ...
        <div className="flex items-center gap-2">
          {user && (
            <>
              <Link href="/">
                <button
                  className="flex items-center gap-1.5 border border-primary/25 bg-primary/8 rounded-lg px-2.5 py-1 text-xs font-mono text-primary/90 hover:border-primary/50 hover:text-primary transition-colors"
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="bg-[#0e0f1e] border-white/8 text-white text-xs min-w-[200px]" align="end">
