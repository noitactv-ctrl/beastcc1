...
export function ProductCard({ product }: { product: ProductWithVariants }) {
  ...
  return (
    <Link href={`/product/${encodeURIComponent(product.name)}`}>
      <div className="group relative cursor-pointer overflow-hidden rounded-xl bg-[#16181d] border border-white/5 transition-all hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/5" data-testid={`card-product-${product.id}`}>
